...
class Message(TelegramObject):
    ...
    def link(self) -> Optional[str]:
        ...
        if self.chat.type not in [Chat.PRIVATE, Chat.GROUP]:
            ...
            to_link = (
                ...
                else f"c/{str(self.chat.id)[4:]}"
            )
            baselink = f"https://t.me/{to_link}/{self.message_id}"

            # adds the thread for topics and replies
            if (
                self.is_topic_message and self.message_thread_id
            ) or self.reply_to_message:
    ...
    def de_json(cls, data: Optional[JSONDict], bot: "Bot") -> Optional["Message"]:
        ...
        data["forward_date"] = from_timestamp(
            data.get("forward_date"), tzinfo=loc_tzinfo
        )
    ...
    def _quote(
        self, quote: Optional[bool], reply_to_message_id: Optional[int]
    ) -> Optional[int]:
        ...
        if quote is not None:
            ...
            if hasattr(self.get_bot(), "defaults") and self.get_bot().defaults:  # type: ignore
                ...
                default_quote = None
            if (
                default_quote is None and self.chat.type != Chat.PRIVATE
            ) or default_quote:
                return self.message_id
        ...
        for entity, text in sorted_entities:
            if entity not in parsed_entities:
                ...
                if entity.type == MessageEntity.TEXT_LINK:
                    ...
                    insert = (
                        f'<a href="tg://user?id={entity.user.id}">{escaped_text}</a>'
                    )
            ...
            if entity not in parsed_entities:
                ...
                if entity.type == MessageEntity.TEXT_LINK:
                    ...
                    code = escape_markdown(
                        text, version=version, entity_type=MessageEntity.PRE
                    )
                    if entity.language:
