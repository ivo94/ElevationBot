...
class TelegramObject:
        ...
        for k in self._get_attrs_names(include_private=True):
            ...
            if k == "api_kwargs":
                ...
                setattr(
                    result,
                    k,
                    MappingProxyType(deepcopy(dict(self.api_kwargs), memodict)),
                )
                continue
    ...
    def _get_attrs_names(self, include_private: bool) -> Iterator[str]:
        ...
        all_attrs = (
            chain(all_slots, self.__dict__.keys())
    ...
    @classmethod
    def de_json(
        cls: Type[Tele_co], data: Optional[JSONDict], bot: "Bot"
    ) -> Optional[Tele_co]:
    ...
    def de_list(
        ...
        return tuple(
            obj for obj in (cls.de_json(d, bot) for d in data) if obj is not None
        )
