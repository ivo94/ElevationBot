    ...
    def decrypted_data(self) -> "Credentials":
        ...
        if self._decrypted_data is None:
            self._decrypted_data = Credentials.de_json(
                decrypt_json(
                    self.decrypted_secret, b64decode(self.hash), b64decode(self.data)
                ),
                self.get_bot(),
            )
...
class DataCredentials(_CredentialsBase):
    ...
    def __init__(
        self, data_hash: str, secret: str, *, api_kwargs: Optional[JSONDict] = None
    ):
...
class FileCredentials(_CredentialsBase):
    ...
    def __init__(
        self, file_hash: str, secret: str, *, api_kwargs: Optional[JSONDict] = None
    ):
