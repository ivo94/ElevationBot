...
class PicklePersistence(BasePersistence[UD, CD, BD]):
    ...
    def _load_singlefile(self) -> None:
        try:
            ...
            raise TypeError(
                f"Something went wrong unpickling {self.filepath.name}"
            ) from exc
    ...
    def _load_file(self, filepath: Path) -> Any:
        try:
            ...
            raise TypeError(
                f"File {filepath.name} does not contain valid pickle data"
            ) from exc
        except Exception as exc:
    ...
    async def update_conversation(
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(
                    Path(f"{self.filepath}_conversations"), self.conversations
                )
            else:
    ...
    async def update_callback_data(self, data: CDCData) -> None:
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(
                    Path(f"{self.filepath}_callback_data"), self.callback_data
                )
            else:
                self._dump_singlefile()
