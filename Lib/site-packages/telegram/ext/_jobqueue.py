...
class JobQueue(Generic[CCT]):
    ...
    def _parse_time_input(
        self,
        time: Union[
            float, int, datetime.timedelta, datetime.datetime, datetime.time, None
        ],
        shift_day: bool = False,
        ...
        if isinstance(time, datetime.time):
            date_time = datetime.datetime.combine(
                datetime.datetime.now(tz=time.tzinfo or self.scheduler.timezone).date(),
                time,
            )
    ...
    def run_once(
        ...
        job = Job(
            callback=callback, data=data, name=name, chat_id=chat_id, user_id=user_id
        )
    ...
    def run_custom(
        ...
        j = self.scheduler.add_job(
            self.job_callback, args=(self, job), name=name, **job_kwargs
        )
