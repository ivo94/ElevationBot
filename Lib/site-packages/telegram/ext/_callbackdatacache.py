...
class CallbackDataCache:
    ...
    * One for mapping the IDs of received callback queries to the cached objects
            ...
            for button in row:
                if button.callback_data:
                    ...
                    (
                        keyboard_id,
                        callback_data,
                    ) = self.__get_keyboard_uuid_and_button_data(button_data)
    ...
    def clear_callback_data(
        self, time_cutoff: Optional[Union[float, datetime]] = None
    ) -> None:
    ...
    def __clear(
        ...
        time_cutoff: Optional[Union[float, datetime]] = None,
    ) -> None:
        if not time_cutoff:
            mapping.clear()
            return
        ...
        to_drop = [
            key for key, data in mapping.items() if data.access_time < effective_cutoff
        ]
        for key in to_drop:
            mapping.pop(key)
