...
class ApplicationBuilder(Generic[BT, CCT, UD, CD, BD, JQ]):
    ...
    def __init__(self: "InitApplicationBuilder"):
        ...
        self._post_init: Optional[
            Callable[[Application], Coroutine[Any, Any, None]]
        ] = None
        self._post_shutdown: Optional[
            Callable[[Application], Coroutine[Any, Any, None]]
        ] = None
        self._post_stop: Optional[
            Callable[[Application], Coroutine[Any, Any, None]]
        ] = None
        ...
        if get_updates and self._get_updates_request is not DEFAULT_NONE:
            raise RuntimeError(  # disallow request args for get_updates if Request for that is set
                _TWO_ARGS_REQ.format(
                    f"get_updates_{name}", "get_updates_request instance"
                )
            )
    ...
    def connection_pool_size(
        self: BuilderType, connection_pool_size: int
    ) -> BuilderType:
    ...
    def connect_timeout(
        self: BuilderType, connect_timeout: Optional[float]
    ) -> BuilderType:
    ...
    def get_updates_proxy_url(
        self: BuilderType, get_updates_proxy_url: str
    ) -> BuilderType:
        """Sets the proxy for the :paramref:`telegram.request.HTTPXRequest.proxy_url`
    ...
    def private_key(
        ...
        self._private_key = (
            ...
            else Path(private_key).read_bytes()
        )
...
InitApplicationBuilder = ApplicationBuilder[  # This is defined all the way down here so that its type is inferred  # by Pylance correctly.
    ExtBot[None],
