...
class Application(Generic[BT, CCT, UD, CD, BD, JQ], AsyncContextManager["Application"]):
    ...
    def __init__(
        ...
        *,
        ...
        post_init: Optional[
            Callable[
                ["Application[BT, CCT, UD, CD, BD, JQ]"], Coroutine[Any, Any, None]
            ]
        ],
        ...
        self.__create_task_tasks: Set[
            asyncio.Task
        ] = set()  # Used for awaiting tasks upon exit
    ...
    async def _add_ch_to_persistence(self, handler: "ConversationHandler") -> None:
        self._conversation_handler_conversations.update(
            await handler._initialize_persistence(
                self
    ...
    def __create_task_done_callback(self, task: asyncio.Task) -> None:
        self.__create_task_tasks.discard(
            task
        ...
        try:
            ...
            # Avoid infinite recursion of error handlers.
                ...
                await self.process_error(
                    update=update, error=exception, coroutine=coroutine
                )
    ...
    async def process_update(self, update: object) -> None:
        ...
        context = None
        any_blocking = (
            False  # Flag which is set to True if any handler specifies block=True
        )
            ...
            try:
                for handler in handlers:
                    ...
                    if not (check is None or check is False):  # if yes,
                        ...
                        if (
                            not handler.block
                            or (  # if handler is running with block=False,
    ...
    def add_handler(
        self, handler: BaseHandler[Any, CCT], group: int = DEFAULT_GROUP
    ) -> None:
        """Register a handler.
    ...
    def migrate_chat_data(
        ...
        if message:
            if (
                message.migrate_from_chat_id is None
                and message.migrate_to_chat_id is None
            ):
                raise ValueError(
    ...
    def remove_error_handler(
        self, callback: HandlerCallback[object, CCT, None]
    ) -> None:
    ...
    async def process_error(
        ...
        if self.error_handlers:
            for (
                ...
                if (
                    not block
                    or (  # If error handler has `block=False`, create a Task to run cb
